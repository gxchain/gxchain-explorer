...
export default {
  ...
  init() {
    return new Promise(resolve => {
      if (current_block_height !== 0) {
        ...
        LevelDBService.get('synced_block_height')
          .then(block_height => {
            current_block_height = block_height || 0
            resolve(current_block_height)
          })
          .catch(ex => {
            console.error('block sync task init failed', ex)
  ...
  sync_to_block(block_height) {
    ...
    if (max_block_height > current_block_height) {
      ...
      self
        .batch_sync_block(parseInt(current_block_height), length)
        .then(function(blocks) {
          ...
          console.log(
            `${current_block_height}/${max_block_height}已同步,同步区块数:`,
            blocks.length
          )
          syncing = false
  ...
  store() {
    return new Promise((resolve, reject) => {
      LevelDBService.put('synced_block_height', current_block_height)
        .then(() => {
          resolve(current_block_height)
        })
        .catch(ex => {
