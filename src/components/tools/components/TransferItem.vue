<template>
    <tr :class="transferColor[data.status]"
        ...
        <td class="text-left"
            style="width:38%;"
            v-if="running">
            <span class="txid-status"
                  v-if="data.status == 'processing' && data.Txid">
                <button type="button"
                        class="btn btn-primary btn-xs"
                        v-if="countDown<=0">
                    <i class="fas fa-spinner"
                       :class="{'loading-animate':transferLoading}"></i>
                </button>
                <button type="button"
                        class="btn btn-primary btn-xs"
                        v-else>
            ...
            <span v-else-if="data.status == 'fail'"
                ...
                <a tabindex="0"
                   ...
                   :class="'popover-toggle-'+index"
                   role="button"
                   data-toggle="popover"
                   data-trigger="focus"
                   data-placement="left"
                   :data-content="data.Mssage">
            ...
            <span v-else-if="data.status == 'success'"
                  class="txid-status">
                <button type="button"
                        class="btn btn-success btn-xs">{{$t('tools.bulk_transfer.success')}}</button>
                <span>
                    Txid:
                    <a target="_blank"
                       :href="'#/transaction/'+data.Txid">{{data.Txid}}</a>
                </span>
            </span>
            <span v-else>
                <div class="text-center loading-animate">
...
export default {
    ...
    methods: {
        get_transaction () {
            ...
            fetch_transaction(this.data.Txid, new Date().getTime())
                .then(res => {
