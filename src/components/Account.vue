  ...
  <div class="container">
    ...
    <section v-if="account_info && account_info.id" v-show="!loading">
        ...
        <div class="col-md-12">
          <div class="panel panel-default">
            <div class="panel-heading">
              ...
              {{ $t('account.basic.title') }}

              <a v-if="network.chainId == '4f7d07969c446f8342033acb3ab2ae5044cbe0fde93db02de75bd17fa8fd84b8'" class="pull-right" :href="'https://wallet.gxb.io/#/account/' + account_info.name + '/overview'" target="_blank">{{ $t('account.basic.more') }}</a>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        <router-link
                          :to="{
                            path: '/account/' + account_info.referrer_name
                          }"
                        >
      ...
      <div class="row">
        <div class="col-md-6">
          <div class="panel panel-default">
            <div class="panel-heading"><span class="fas fa-gavel"></span>&nbsp;{{ $t('account.permissions.title') }}</div>
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-striped no-margin">
                  <tbody>
                    <tr class="active">
                      <th width="80%">
                        <span class="fa fa-lock">&nbsp;{{ $t('account.permissions.active') }}</span>
                      </th>
                    ...
                    <tr class="active">
                      <th>
                        <span class="fa fa-lock">&nbsp;{{ $t('account.permissions.owner') }}</span>
                      </th>
        ...
        <div class="col-md-6">
          <div class="panel panel-default">
            <div class="panel-heading"><span class="fa fa-fw fa-money-check"></span>&nbsp;{{ $t('account.balances.title') }}</div>
            <div class="panel-body no-padding">
              <table class="table table-striped no-margin">
                <thead>
                  <tr>
          ...
          <div class="panel panel-default panel-contract">
            <div class="contract-info-wrap">
              <div class="pull-right tool-tip">
                <span class="fa fa-fg fa-lightbulb" data-toggle="tooltip" data-placement="top" :title="$t('account.contract.tooltip')"></span>
              </div>
              <div class="contract-info">
                <img v-if="contractInfo && contractInfo.logo" class="portrait" :src="contractInfo.logo" alt="" />
                <svg v-if="!(contractInfo && contractInfo.logo)" class="portrait" id="icon-contract" viewBox="0 0 1024 1024" width="100%" height="100%">
                  <path d="M512 0a512 512 0 1 1 0 1024A512 512 0 0 1 512 0z m38.115556 284.444444l-169.528889 455.111112h93.297777l169.528889-455.111112H550.115556z m108.771555 134.257778L796.444444 512l-163.271111 110.762667 50.176 34.019555L853.333333 541.809778V482.986667l-176.184889-115.825778-18.261333 51.484444zM170.666667 482.190222v58.766222l176.184889 115.825778 18.261333-51.484444L227.555556 512l163.271111-110.762667-50.232889-34.019555L170.666667 482.190222z" fill="#34CB49"></path>
                </svg>
                <div v-if="contractInfo">
                  <div class="account-wrap">
                    ...
                    <a v-if="contractInfo.links && contractInfo.links.github" :href="contractInfo.links.github" target="_blank">
            ...
            <div class="panel-heading">
              <span class="tab" :class="{ active: contractTab == item.key }" v-for="(item, index) in contractTabs" :key="item.key" @click="selectSontractTab(item.key)"> <span class="fas" :class="[item.icon]"></span>&nbsp;{{ item.name }} </span>
            </div>
            <!--ABI-->
              ...
              <div class="tab-content">
                      ...
                      <tbody>
                        <tr v-for="action in account_info.abi.actions" :key="action.name">
                          ...
                          <td align="right" v-if="gxc">
                            <a class="btb btn-xs btn-success" href="javascript:;" @click="setContractParams(action)">call</a>
                          </td>
                        </tr>
                      </tbody>
                    </table>
                ...
                <div role="tabpanel" class="tab-pane" :class="{ active: abi.type == 'table' }">
                  <div class="panel panel-default panel-tables" v-for="table in getTableDefine(account_info.abi.tables)">
                    <div class="panel-heading"><span class="fa fa-fw fa-list"></span> {{ table.name }}</div>
                    <div class="table-responsive">
                      <table class="table table-striped">
                        <thead>
                          <tr>
            ...
            <div class="panel-body no-padding" v-show="contractTab == 'DATABASE'">
              <div class="panel-heading panel-heading-database">
                ...
                <a class="btn btn-sm btn-default" :class="{ disabled: current_table.page === 0 }" @click="loadTableData(current_table.page - 1)">
                  <i class="fa fa-arrow-left"></i>
                </a>
                <a class="btn btn-sm btn-default" :class="{ disabled: !current_table.hasMore }" @click="loadTableData(current_table.page + 1)">
      ...
      <div class="row" v-if="!is_contract_account">
        <div class="col-md-12">
          <div class="panel panel-default">
            <div class="panel-heading"><span class="fa fa-fw fa-history"></span>&nbsp;{{ $t('account.staking.title') }}</div>
            <div class="panel-body no-padding">
                ...
                <table class="table table-bordered table-striped no-margin">
                  ...
                  <tbody>
                    <tr v-if="stakings.length > 0" v-for="staking in stakings" :class="{ success: !staking.is_valid }">
                      <td>{{ staking.id }}</td>
                      <td>
                        <router-link :to="{ path: '/account/' + formatted_account(staking.trust_node) }">
                          {{ formatted_account(staking.trust_node) }}
                        </router-link>
                      </td>
                      <td class="right">
                        {{ formatted_asset(staking.amount.asset_id, staking.amount.amount) }}
                      </td>
        ...
        <div class="col-md-12">
          <div class="panel panel-default">
            <div class="panel-heading">
              ...
              <a class="btn btn-sm btn-default" :class="{ disabled: awarded_stakings.starts.length < 3 }" @click="awarded_stakings.starts.pop() && awarded_stakings.starts.pop() && loadAwardedStakings()">
                <i class="fa fa-arrow-left"></i>
              </a>
              <a class="btn btn-sm btn-default" :class="{ disabled: !awarded_stakings.more }" @click="loadAwardedStakings()">
                <i class="fa fa-arrow-right"></i>
              </a>
              <span class="pull-right fa fa-fg fa-lightbulb" data-toggle="tooltip" data-placement="top" :title="$t('account.staking.tooltip')"> </span>
            </div>
            <div class="panel-body no-padding">
                ...
                <table class="table table-striped table-bordered no-margin" v-show="!history_loading">
                  <thead>
                    <tr>
                      ...
                      <th class="right" width="80">
                        {{ $t('index.transactions.time') }}
                      </th>
                    </tr>
                  </thead>
    ...
    <modal-call-contract ref="modalCall" :contract="contract.name" :method="contract.method" :payable="contract.payable" :fields="contract.params"> </modal-call-contract>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getWAST() {
      this.$http
        .post('/api/wasm2wast', { wasm: this.account_info.code })
        .then((resp) => {
